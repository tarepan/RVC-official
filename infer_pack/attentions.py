...
class Encoder(nn.Module):
    ...
    def __init__(self, hidden_channels: int, filter_channels: int, n_heads, n_layers, kernel_size: int=1, p_dropout=0.0, window_size=10):
        ...
        Args:
            ...
            filter_channels - FF block's middle layer width
            n_heads         - The number of attention heads
            n_layers        - The number of Encoder block
            kernel_size     - Conformer FF block's conv kernel size (k=1 means SegFC == pure Transformer, not Conformer)
    ...
    def forward(self, x, x_mask):
        ...
        x = x * x_mask
        for i in range(self.n_layers):
            # Res[MHA-Do]-Norm-Res[FF-Do]-Norm
            ## Attention
