...
class Pipeline(object):
    def __init__(self, tgt_sr, config):
        self.x_pad, self.x_query, self.x_center, self.x_max, self.is_half = (config.x_pad, config.x_query, config.x_center, config.x_max, config.is_half)
    ...
    def get_f0(self, input_audio_path, x, p_len, f0_up_key, f0_method, filter_radius, inp_f0=None):
        """Extract and convert pitch series from audio.

        Args:
            input_audio_path - (maybe) Audio file name
            x                - (maybe) Waveform
        Returns:
            f0_coarse
            f0bak
        ...
        ## CREPE
            ...
            f0, pd = torchcrepe.predict(audio, self.sr, self.window, f0_min, f0_max, model, batch_size=batch_size, device=self.device, return_periodicity=True)
    ...
    def vc(self, model, net_g, sid, audio0, pitch, pitchf, times, index, big_npy, index_rate, version, protect):  # ,file_index,file_big_npy
        """RVC voice conversion, Wave-to-Unit-to-RetrievalUnit-to-Wave.

        Args:
            model      - (maybe) Feature extractor
            net_g      - Unit-to-Wave generator
            sid : LongTensor - (maybe) Speaker ID
            audio0     - Source audio waveform
            pitch  : NDArray - Already-converted coarse pitch series
            pitchf : NDArray - Already-converted fine   pitch series
            times      - Processing time log container
            index      - Retrieval things
            big_npy    - Retrieval things
            index_rate - Retrieval things
            version : Literal["v1"] | Any
            protect
        Returns:
            audio1 : NDArray[np.float32] - Generated audio
        """

        # Prepare - Tensor/type/shape/device/enum
        ...
        # Retrieval :: feats: -> feats: - Unit retrieval
        ...
        # Generation
        with torch.no_grad():
            hasp = pitch is not None and pitchf is not None # maybe means 'has pitch'
    ...
    def pipeline(self, model, net_g, sid, audio, input_audio_path, times, f0_up_key, f0_method, file_index, index_rate, if_f0, filter_radius, tgt_sr, resample_sr, rms_mix_rate, version, protect, f0_file=None):
        """Prepare retrieval index, audio chunks, pitch series, then convert pitch, finally run `self.vc` and postprocess the output.

        Args:

            sid        - (maybe) Speaker ID

            index_rate - Retrieval things
            if_f0      - Whether to use explicit pitch

            file_index - (maybe) Path of retrieval index file
        """

        # Retrieval things - Acquire `index` & `big_npy` from `file_index` & `index_rate`
        ...
        # (maybe) Speaker ID :: LongTensor
        sid = torch.tensor(sid, device=self.device).unsqueeze(0).long()

        # Pitch extraction & conversion - Coarse pitch series (`pitch`) and Fine-grained pitch series (`pitchf`)
        ...
        for t in opt_ts:
            ...
            if if_f0 == 1:
                audio_opt.append(self.vc(model, net_g, sid, audio_pad[s : t + self.t_pad2 + self.window], pitch[:, s // self.window : (t + self.t_pad2) // self.window], pitchf[:, s // self.window : (t + self.t_pad2) // self.window], times, index, big_npy, index_rate, version, protect)[self.t_pad_tgt : -self.t_pad_tgt])
            else:
