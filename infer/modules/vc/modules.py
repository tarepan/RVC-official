...
class VC:
    def __init__(self, config):
        ...
        self.pipeline = None # Core runner
        self.cpt = None
        self.version = None
        self.if_f0 = None
        self.version = None
    ...
    def get_vc(self, sid, *to_return_protect):
        ...
        to_return_protect0 = {"visible": self.if_f0 != 0, "value": to_return_protect[0] if self.if_f0 != 0 and to_return_protect else 0.5, "__type__": "update"}
        to_return_protect1 = {"visible": self.if_f0 != 0, "value": to_return_protect[1] if self.if_f0 != 0 and to_return_protect else 0.33, "__type__": "update"}

        # (Early return)
        if sid == "" or sid == []:
            if self.hubert_model is not None:  # 考虑到轮询, 需要加个判断看是否 sid 是由有模型切换到无模型的 Considering polling, a judgment needs to be added to see whether the sid is switched from model to modelless.
                ...
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()
            return ({"visible": False, "__type__": "update"}, {"visible": True, "value": to_return_protect0, "__type__": "update"}, {"visible": True, "value": to_return_protect1, "__type__": "update"}, "", "")
        ...
        # Generator Network
        ## Class: f"SynthesizerTrnMs{256 if [0] == 'v1' else '768'}NSFsid{'' if [1] == 1 else '_nono'}"
        synthesizer_class = {("v1", 1): SynthesizerTrnMs256NSFsid, ("v1", 0): SynthesizerTrnMs256NSFsid_nono, ("v2", 1): SynthesizerTrnMs768NSFsid, ("v2", 0): SynthesizerTrnMs768NSFsid_nono}
        ...
        return ({"visible": True, "maximum": n_spk, "__type__": "update"}, to_return_protect0, to_return_protect1, index, index) if to_return_protect else {"visible": True, "maximum": n_spk, "__type__": "update"}
    ...
    def vc_single(self, sid, input_audio_path, f0_up_key, f0_file, f0_method, file_index, file_index2, index_rate, filter_radius, resample_sr, rms_mix_rate, protect):
        ...
        try:
            ...
            file_index = (file_index.strip(" ").strip('"').strip("\n").strip('"').strip(" ").replace("trained", "added")) if file_index != "" else file_index2  # 防止小白写错，自动帮他替换掉
                ...
                if "Success" in info:
                    try:
                        ...
                        if format1 in ["wav", "flac"]:
                            sf.write("%s/%s.%s" % (opt_root, os.path.basename(path), format1), audio_opt, tgt_sr)
                        else:
